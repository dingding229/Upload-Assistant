...
BDMV scanning uses the bdinfo build from [the project repository](https://github.com/dingding229/bdinfo). Install its `bdinfo` executable on the VPS and make it available on `PATH`, or place it at `bin/bdinfo/bdinfo`. You can also set `BDINFO_PATH` to an executable at another location.
...
Built with [bdinfo](https://github.com/dingding229/bdinfo).
