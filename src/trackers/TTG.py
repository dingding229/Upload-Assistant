...
class TTG:
    ...
    async def edit_desc(self, meta: Meta) -> None:
        ...
        if meta.get('discs', []) != []:
            ...
            for each in discs:
                if each['type'] == "BDMV":
                    summary = str(each.get('summary', '') or '').strip()
                    # DESCRIPTION.txt may already contain the BDInfo block
                    # generated by the common description builder. Do not add
                    # the same summary a second time for TTG.
                    if summary and summary not in base:
                        # TTG expects the BDInfo block to use the plain quote
                        # tag, matching CHDBITS/HDSKY formatting.
                        parts.append(f"[quote]{summary}[/quote]\n")
                        parts.append("\n")
                if each['type'] == "DVD":
